...
- **Workspace load on every run.** `analyze` loads the cargo workspace via `ra_ap_load_cargo` (rust-analyzer-as-library) to give the `unused` detector name resolution that resolves through macros, disambiguates homonyms, and follows re-exports. Per-run cost is comparable to `cargo clippy` on the same workspace (~15-20 s for a 60-file workspace, dominated by `cargo metadata` + macro-server setup). First-time install also pulls the `ra_ap_*` dep tree (~170 transitive crates, ~50 s cold build).
- **Most metric lenses are still AST-only.** Function-shape lenses (`panic-density`, `lifetime-arity`, `generic-arity`, `unsafe-block-scope`, `iterator-chain-length`, SLOC, …) read `ra_ap_syntax` per file and don't need name resolution. The complexity / coupling / cohesion lenses (CC, Cognitive, NPath, LCOM4, RFC, the Martin trio) are mid-migration to HIR-aware variants; see [`tmp/hir-default-plan.md`](tmp/hir-default-plan.md) for the per-lens status.
- **Not a fit if** you need per-line metric thresholds in the IDE for the full metric suite, you don't engage with the dismiss channel at all (a pure-fail-fast linter is a better fit), or your codebase relies heavily on proc-macro / derive output that lives in macro bodies un-resolved code paths cannot reach (run with `--expanded-macros` to feed `cargo expand`'s output to the per-file walkers).
