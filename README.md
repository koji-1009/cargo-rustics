...
- **Per-file Martin granularity** is brittle for Rust. The release unit in Rust is the *crate*, not the file, and a `.rs` file can hold any number of `pub` items — Martin's 1994 framework assumes Java-style "1 public class per file" which doesn't hold. `efferent-coupling` / `afferent-coupling` / `instability` ship as change-impact rankings rather than Pain/Uselessness verdicts; `abstractness` and `distance-from-main-sequence` are dropped. See [`doc/calibration.md`](doc/calibration.md).
