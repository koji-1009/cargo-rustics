...
- **Coverage gate covers the whole workspace, target 100 %.** CI runs `cargo llvm-cov --workspace --fail-under-lines 90`. Plan §12.5 mandates 100 % line coverage on `lib/`; the 90 % gate is a regression guard while individual crates are ratcheted toward 100 %. Lines that are *structurally* uncoverable from inside the same crate are listed below — every other gap is a missing test.
   - `rustics-macros/src/lib.rs` — the `#[proc_macro_attribute]` entry point can't be unit-tested from inside its own crate (proc-macro tokens are only valid during compilation of *another* crate). Helper functions (`op_holds`, `ident_to_metric_id`, `compute_measurements`, `check_constraints`) are fully covered.
   - `rustics-lsp/src/main.rs` — the `main()` function spins up `Connection::stdio()` which is uncovered without a real LSP peer. `main_loop`, `handle_notification`, `compute_diagnostics`, `severity_from_lens`, `diagnostic_for`, `publish_diagnostics` are all covered against `Connection::memory()`.
   - `cargo-rustics/src/expanded.rs` — drives the `cargo expand` subprocess; only reachable on a host with `cargo-expand` installed. CI hosts may have it; fallback paths (process-fail handling) are covered.
