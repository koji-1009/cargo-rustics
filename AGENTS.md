...
- `crates/rustics/src/{input,measurement,metric,scope,visitor,identifier}.rs` — AST-walker seams: `MetricInput`, `MetricMeasurement`, the `MetricCalculator` trait + metadata enums, `ScopeRef`, the `walk_*` / `measure_*` helpers, the `violation_id` hash. Used by every lens whose definition is "walk a single parsed `SourceFile`" — function-shape lenses (CC, panic-density, lifetime-arity, …) live here. Lenses that need name resolution dispatch through `crates/rustics-ra/` instead.
- `crates/rustics-ra/src/{lib,workspace,unused}.rs` — the rust-analyzer-as-library backend. `workspace::load` wraps `ra_ap_load_cargo` (macro server in `Sysroot` mode); `unused::detect_at` walks HIR for the public-API reachability heuristic, using `Definition::usages` for cross-crate / through-macro reference resolution. Strong-gain lenses (per `tmp/ra-ap-spike-notes.md`: CC / Cognitive / NPath / LCOM4 / RFC / coupling trio / match-arm-count) are mid-migration to HIR — see `tmp/hir-default-plan.md` for the per-lens status.
